...
import { stepInstruction, useStepGuidance } from './composables/useStepGuidance.js';
...
const voiceText = computed(() => (
  activeStep.value ? stepInstruction(activeStep.value) : VOICE_TEXTS[String(route.name)] ?? null
));
const guidanceReady = computed(() => !store.financeLoading && !store.inquiryLoading && !store.supportLoading);
const { notice: guidanceNotice, handleClick: handleGuidanceClick } = useStepGuidance(
  routeArea, activeStep, guidanceReady,
  () => {
    if (store.currentStepVisit?.stepId === activeStep.value?.stepId) store.recordPatternAction('wrongTouch');
  },
);
